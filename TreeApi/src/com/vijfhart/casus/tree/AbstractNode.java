...
class AbstractNode<E extends Node<E>> implements Node<E> {//, LevelComparable<AbstractNode>, Comparator<AbstractNode> {
	private String naam;
	//private E nameNode;
	...
	public int compareTo(E node) {
		...
		if(getParent() == node.getParent()) {
			...
			set.add(this);
			set.add(node);
			if(set.first() == this) {return -1;} else {return 1;}
		...
		if(this.getParent() != node & node.getParent()!= this & this.compareLevelTo(node) != 0) {
			if(((AbstractNode)this.getParent()).compareLevelTo(((AbstractNode)node).getParent()) < (node.getParent()).compareLevelTo(this.getParent())) {
	...
	public int compareLevelTo(E node) {
		...
		if(getParent() == null) return -1;
		if((getParent()).equals(node.getParent())) {return 0;}
    ...
    private boolean isDescendant(E node) {
	  ...
	  if(node.compareLevelTo((E)this) > 0) return true;
